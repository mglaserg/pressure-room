# Pressure Room GitHub Actions AWS OIDC setup - v2
...
if ($RoleExists) {
    ...
    Assert-AwsSuccess 'Failed to update the deploy role trust policy.'
    Write-Host '      Existing role updated.'
}
else {
    aws iam create-role --role-name $RoleName --description 'GitHub Actions OIDC deploy role for Pressure Room' --assume-role-policy-document ('file://' + $TrustPath) --output json | Out-Null
    Assert-AwsSuccess 'Failed to create the deploy role.'
...
Assert-AwsSuccess 'Failed to attach the Pressure Room deployment policy.'
