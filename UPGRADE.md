...
This patch applies to uploaded commit `94744ca9860b9fb77c82477f0afb9904cb3c1af0`. It resolves committed merge-conflict markers, retaining the v0.7.0 implementation, removes duplicated export limit definitions and adds the 0.7.1 episode fix. Apply with `git apply --check <patch-path>` followed by `git apply <patch-path>`, then rebuild/restart the frontend. The full bundle and ZIP include all earlier changes. No new database migration is introduced.

The earlier upgrade and deployment requirements follow for users installing the full release from an older version.
