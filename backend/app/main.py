...
app = FastAPI(title="Pressure Room API", version="0.4.3", lifespan=lifespan)
app.add_middleware(
    CORSMiddleware,
...
def create_project(payload: Payload):
    d = payload.data
    ts = db.now_iso()
    pid = db.insert("projects", {"title": d.get("title", "Untitled Story").strip() or "Untitled Story", "premise": d.get("premise", ""), "theme": d.get("theme", ""), "created_at": ts, "updated_at": ts})
...
def patch_object(table: str, object_id: str, payload: Payload):
...
def delete_object(table: str, object_id: str):
...
def create_character(project_id: str, payload: Payload):
    ts = db.now_iso(); d = payload.data
    cid = db.insert("characters", {"project_id": project_id, "name": d.get("name", "New Character"), "role": d.get("role", ""), "want": d.get("want", ""), "need": d.get("need", ""), "core_belief": d.get("core_belief", ""), "moral_boundary": d.get("moral_boundary", ""), "fear": d.get("fear", ""), "temptation": d.get("temptation", ""), "moral_score": int(d.get("moral_score", 0)), "created_at": ts, "updated_at": ts})
    return {"id": cid}


@app.post("/api/projects/{project_id}/episodes")
def create_episode(project_id: str, payload: Payload):
    d = payload.data; ts = db.now_iso()
    branch_id = d.get("branch_id") or db.one("SELECT id FROM branches WHERE project_id=? AND is_main=1 LIMIT 1", [project_id])["id"]
    eid = db.insert("episodes", {"project_id": project_id, "branch_id": branch_id, "number": int(d.get("number", 1)), "title": d.get("title", "Untitled Episode"), "logline": d.get("logline", ""), "status": d.get("status", "Outline"), "created_at": ts, "updated_at": ts})
    return {"id": eid}


@app.post("/api/episodes/{episode_id}/scenes")
def create_scene(episode_id: str, payload: Payload):
    d = payload.data; ts = db.now_iso()
    sid = db.insert("scenes", {"episode_id": episode_id, "scene_no": int(d.get("scene_no", 1)), "slugline": d.get("slugline", ""), "pov_character_id": d.get("pov_character_id"), "opening_behavior": d.get("opening_behavior", ""), "scene_want": d.get("scene_want", ""), "obstacle": d.get("obstacle", ""), "tactic": d.get("tactic", ""), "pressure": d.get("pressure", ""), "choice": d.get("choice", ""), "start_state": d.get("start_state", ""), "end_state": d.get("end_state", ""), "cut_on": d.get("cut_on", ""), "notes": d.get("notes", ""), "screenplay_text": d.get("screenplay_text", ""), "moral_delta": int(d.get("moral_delta", 0)), "created_at": ts, "updated_at": ts})
    return {"id": sid}
...
def create_bill(project_id: str, payload: Payload):
    d = payload.data; ts = db.now_iso()
    bid = db.insert("bills", {"project_id": project_id, "episode_id": d.get("episode_id"), "scene_id": d.get("scene_id"), "character_id": d.get("character_id"), "title": d.get("title", "Untitled Bill"), "external_cost": d.get("external_cost", ""), "moral_cost": d.get("moral_cost", ""), "status": d.get("status", "Outstanding"), "payoff_scene_id": d.get("payoff_scene_id"), "created_at": ts, "updated_at": ts})
    return {"id": bid}


@app.post("/api/projects/{project_id}/branches/{branch_id}/clone")
def clone_branch(project_id: str, branch_id: str, payload: NamePayload):
    try:
        return {"id": db.clone_branch(project_id, branch_id, payload.name)}
    except KeyError:
        raise HTTPException(404, "Branch not found")


@app.post("/api/projects/{project_id}/snapshots")
def create_snapshot(project_id: str, payload: SnapshotPayload):
    return {"id": db.create_snapshot(project_id, payload.label)}


@app.post("/api/snapshots/{snapshot_id}/restore")
def restore_snapshot(snapshot_id: str):
    try:
        return {"project_id": db.restore_snapshot(snapshot_id)}
    except KeyError:
        raise HTTPException(404, "Snapshot not found")


@app.get("/api/episodes/{episode_id}/diagnostics")
def diagnostics(episode_id: str):
    ...
    return {"mri": story_mri(scenes, bills), "questions": writers_room_questions(protagonist, scenes, bills), "pressure_moves": [{"name": a, "description": b} for a, b in PRESSURE_MOVES]}
...
@app.post("/api/import")
async def import_project(file: UploadFile = File(...), mode: str = Query("copy", pattern="^(copy|replace)$")):
    raw = await file.read()
    try:
