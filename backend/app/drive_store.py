...
def upload_text(session: dict, file_id: str, text: str) -> None:
    meta = file_metadata(session, file_id)
    can_edit = (meta.get("capabilities") or {}).get("canEdit")
    if can_edit is False:
        raise HTTPException(403, "The selected Fountain file is read-only.")
    _request(
        session,
        "PATCH",
...
def save_project(session: dict, project_id: str) -> dict:
    ...
    with SYNC_LOCK:
        ...
        sidecar_id = source.get("sidecar_file_id") if source else None
        current = None
        if sidecar_id:
            current = next((item for item in files if item.get("id") == sidecar_id), None)
        if not current:
            current = next(
                (
                    item
        ...
        filename = (
            source.get("sidecar_file_name")
            if source and source.get("sidecar_file_name")
            else _safe_filename(payload["project"]["title"])
        )
        metadata = {
            "name": filename,
            "appProperties": {
                ...
                "pressure_room_source_kind": (source or {}).get("source_kind", ""),
            },
        }

        file_id = current.get("id") if current else None
        if file_id:
            try:
                _request(
                    session,
                    "PATCH",
                    f"{DRIVE_FILES_URL}/{file_id}",
                    params={"fields": "id,name,modifiedTime,appProperties,parents"},
                    json=metadata,
                )
        ...
        if not file_id:
            preferred_parent = (source or {}).get("drive_parent_id")
            parent_id = preferred_parent or _folder_id(session)
            create_metadata = dict(metadata)
            create_metadata["parents"] = [parent_id]
            try:
                created = _request(
                    session,
                    "POST",
                    DRIVE_FILES_URL,
                    params={"fields": "id,name,modifiedTime,appProperties,parents"},
                    json=create_metadata,
                ).json()
            except HTTPException:
                if not preferred_parent:
                    raise
                # A selected file can be authorized without its parent folder being
                # writable by the app. Fall back to Pressure Room/ rather than fail.
                fallback = dict(metadata)
                fallback["parents"] = [_folder_id(session)]
                created = _request(
                    session,
                    "POST",
                    DRIVE_FILES_URL,
        ...
        if source:
            db.upsert_project_source(
                project_id,
                sidecar_file_id=file_id,
                sidecar_file_name=filename,
