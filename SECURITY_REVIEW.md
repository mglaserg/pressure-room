...
Verified with the production build and targeted Chromium checks: existing-story episode creation on desktop/mobile; saved draft preservation; correct path and next episode number; automatic selection; first and second episodes on an empty path. Results: `audit/episode-results.json`; reproducible check: `scripts/episode-check.cjs`, using the same Playwright environment settings as `scripts/browser-check.cjs`.

Dependency versions are unchanged. The prior audit below remains the security evidence for this release; no new live Google/AWS checks are claimed.
